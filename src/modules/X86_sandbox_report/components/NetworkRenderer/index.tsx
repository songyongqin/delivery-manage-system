...
const itemKeyList = [USUAL_C2, DYNAMIC_C2, HTTP, TCP, UDP, DNS, HOST]
    ...
    renderer: {
      [TAINTED_INFO]: (value = []) => {
        return <div>
          {
            value.map((i, index) => {
              return <Tag key={`${index}-item`} color={primaryColor}>{i}</Tag>
            })
          }
        </div>
      }
  ...
  return <div>
    {
      finalItemKeys.map(i => {
        ...
        return <div key={`${i}-item`} style={{ marginBottom: "15px" }}>
          <h3 style={{ color: primaryColor, paddingLeft: "15px" }}>{itemTitleConfig[i]}</h3>
          <EnhancedTable
            tableProps={{
