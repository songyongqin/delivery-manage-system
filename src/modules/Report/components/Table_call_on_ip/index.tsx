    ...
    return (
      <div>
        <JoSpin spinning={this.props.loading}>
          <h4 className={classnames({ "lbl-dark": isDark })} style={{ textAlign: "center", marginBottom: "25px", marginTop: "50px" }}>访问的外网IP</h4>
          <div style={{ position: "absolute", top: "0px", right: "0px" }} >
            <Tooltip_ title={TOOLTIP_TITLE_IP} placement="left">
...
function mapStateToProps(state) {
  const { data, loading, timestampRange, page, limit, total } = state[NAMESPACE_CALL_ON_IP];
  return {
    data,
    loading: state.loading.effects[`${NAMESPACE_CALL_ON_IP}/fetch`],
    timestampRange,
    page,
    limit,
    total,
