...
export default function LoadingButton(el, options){
  this.el = el;
...
LoadingButton.prototype = {
  ...
  initEvents: function(){
    ...
    //   classie.add(self.el, styles['open-loading']);
  ...
  successAnimation: function(){
    ...
    self.successSegment.draw('100% - 50', '100%', 0.4, {callback: function(){
      classie.add(self.el, styles['succeed']);
  ...
  errorAnimation: function(){
    ...
    self.errorSegment2.draw('100% - 42.5', '100%', 0.4, {callback: function(){
      classie.add(self.el, styles['failed']);
      // setTimeout(function(){ self.reset(); }, 2000);
    }});
  },
  ...
  reset: function(){
    ...
    classie.remove(this.el, styles['open-loading']);
    classie.remove(this.el, styles['failed']);
    classie.remove(this.el, styles['succeed']);
    this.resetSegments();
    this.infinite = true;
    for(var i = 0, paths = this.options.paths, len = paths.length; i < len; i++){
